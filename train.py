...
from segm_benchmark.solver import make_lr_scheduler
from segm_benchmark.solver import make_optimizer
from segm_benchmark.engine.inference import inference
from segm_benchmark.engine.trainer import do_train
...
from segm_benchmark.utils.miscellaneous import mkdir, save_config
...
def run_val(cfg, model, distributed):
    if distributed:
        model = model.module
    ...
    data_loader_val = make_data_loader(
        cfg, stage='val', is_distributed=distributed)
    inference(
        model,
        data_loader_val,
...
def main():
    ...
    logger = setup_logger("segm_benchmark", output_dir, get_rank())
    ...
    if not args.skip_test:
        run_val(cfg, model, args.distributed)
